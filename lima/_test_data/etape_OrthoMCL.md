...
Lien de téléchargement d'OrthoMCL:
...
(7) run all-v-all NCBI BLAST on goodProteins.fasta (output format is tab delimited text).
	Théorie
		La stratégie suivante a été adoptée à cette étape:
			a) créer une base de données regroupant toutes les protéines des 9 espèces étudiées.
			b) exécuter blast avec chaque espèce contre la base de données.
			c) fusionner les sorties obtenues dans un seul fichier.
	...
	Application
		...
		b) exécution de BLAST pour chaque espèce contre la même base de données:
		...
		c) Concaténation des fichiers de sortie de BLASTP
			Commande :
